...
async def _ensure_user_columns() -> None:
    ...
    engine = create_async_engine(settings.database_url)
    stmts = (
        str(cfg["sql_add_user_xp"]),
        str(cfg["sql_add_user_rank"]),
        str(cfg["sql_add_user_games"]),
        str(cfg["sql_add_user_wins"]),
        str(cfg["sql_add_user_bio"]),
        str(cfg["sql_add_user_username"]),
        str(cfg["sql_add_web_status"]),
    )
    async with engine.begin() as conn:
        for stmt in stmts:
            await conn.execute(text(stmt))
