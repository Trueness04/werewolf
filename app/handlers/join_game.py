...
async def _pv_send(
    ...
    """Send a PV message; ignore Forbidden (user blocked bot)."""
    try:
...
async def run_join_steps(
    ...
    if other and int(other) != chat_id:
        await _pv_send(context, user.id, tm.get("YouInGame", lang))
        log_game_event(
    ...
    # 4 allow
    if allow <= 0:
        await _pv_send(context, user.id, tm.get("NotAllowToJoin", lang))
        log_game_event(
            "join_fail_allow",
            chat_id=chat_id,
            user_id=user.id,
    ...
    if await lobby.name_taken(chat_id, fullname):
        await _pv_send(
            context,
            user.id,
            tm.get("NotNameAllow", lang, fullname),
        )
        log_game_event(
            "join_fail_name",
            chat_id=chat_id,
    ...
    if not exists:
        await _pv_send(context, user.id, tm.get("NotFoundGameId", lang))
        log_game_event(
            "join_fail_missing",
            chat_id=chat_id,
            user_id=user.id,
